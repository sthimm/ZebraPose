#modified from Cosypose: https://github.com/ylabbe/cosypose
...
def getXYZ(depth,fx,fy,cx,cy,bbox=np.array([0])):
    ...
    #uv_table = np.zeros((depth.shape[0],depth.shape[1],2))
...
def icp_refinement(depth_measured, depth_rendered,
                   object_mask_measured, cam_K, TCO_pred, n_min_points=1000, n_iters=100):
    ...
    icp_fnc = cv2.ppf_match_3d_ICP(n_iters, tolerence=tolerence, numLevels=4)
